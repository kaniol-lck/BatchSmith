...
[[nodiscard]] QString render_preset() {
    return QStringLiteral(
            ...
            "挑一个目录即可（那里也能给它挑个图标）。</li>"
            "<li><b>备注</b>：预设里可以写一行 <code>note = '...'</code>"
            "（「管理预设 → 备注…」改的就是它）。它是给人看的一句话，会跟着预设一起分享出去。</li>"
            "<li><b>图标</b>：管理预设里能给预设挑一个图标，建快捷方式时用它。图标是"
            "<b>本机路径</b>，所以写在旁边的 <code>.local.toml</code> 里、不进预设文件 —— "
            "预设照样可以干净地发给别人。</li>"
