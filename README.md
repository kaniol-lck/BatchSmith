...
会先问你放到哪个目录，然后生成一个带着 `--preset` 参数的快捷方式
（Windows `.lnk` / Linux `.desktop` / macOS `.command`），双击就是"用这套配置打开 BatchSmith"。
同一个对话框里还能改**备注**（存进预设文件，会跟着预设分享出去）与**图标**
（本机路径，存进 `.local.toml`，不进预设文件），并且能直接看到每个预设的**输出表达式**。

**预设里不存绝对路径**：绑定的文件夹写成槽位 `${input}`，本机实际路径存在旁边的
`<预设名>.local.toml` 里 —— 于是预设文件可以进版本控制、可以发给别人
（分享时别带那个 `.local.toml`）。

常用表达式片段（完整清单见 F1 速查表）：

| 写法 | 说明 |
|---|---|
| `$regex(名, [[第(\d+)话]], 1)$` | 正则捕获，第三个参数直接取第 1 项（越界给空串） |
| `$replace(名, "_final", "")$` | **字面**全局替换（不认正则） |
| `$resub(名, [[第(\d+)话]], [[第\1话 正片]])$` | 正则全局替换，`\1` 引用捕获组 |
